...
| Keyed workload | Solid 2.0 | solidlil | Ratio |
| --- | ---: | ---: | ---: |
...
LilScript apps are written in **LSX** (`.lilx`) — JSX for LilScript — and compile closed-world. JavaScript consumers use the tuple helpers on the default entry. A DOM app should import only from `solidlil/web` so it shares one reactive graph; mixing `solidlil` with `solidlil/web` duplicates the runtime.
...
```js
import { render, bindText, keyedEach, createIntSignal } from "solidlil/web"
```
