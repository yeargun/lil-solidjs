  ...
  if (node.type === "element" && isComponent(node.tag)) {
    if (isBuiltinComponent(node.tag)) {
      ...
      return {
        ...
        code: [
          ...lowered.code,
          `Element ${root} = materializeNodeGroup(${lowered.variable});`,
        ],
      };
    }
  ...
  const constructor =
    namespace === "svg"
      ...
      : namespace === "math"
        ...
        : "element";
  const code = [
    `Element ${variable} = ${constructor}(${JSON.stringify(node.tag)});`,
  ];
...
function lowerBuiltinChild(node, context, parentVariable, namespace) {
  ...
  if (node.tag === "Repeat")
    return lowerRepeat(node, context, parentVariable, namespace);
  if (node.tag === "Switch")
    return lowerSwitch(node, context, parentVariable, namespace);
  if (node.tag === "ErrorBoundary" || node.tag === "Errored")
    return lowerErrorBoundary(node, context, parentVariable, namespace);
  if (node.tag === "Suspense" || node.tag === "Loading")
...
function lowerComponent(node, context, parentVariable, namespace = "html") {
  ...
  code.push(`Element ${component} = componentNode(${node.tag}, ${props});`);
...
function lowerComponentNodeGroup(node, context, namespace = "html") {
  ...
  code.push(`Element[] ${nodes} = componentNodes(${node.tag}, ${props});`);
...
function lowerShow(node, context, parentVariable, namespace = "html") {
  ...
  const code = [
    `dynamicShow(region(${parentVariable}), () => ${condition.trim()}, () => {`,
    ...children.code.map((line) => `  ${line}`),
    `  Element[] nodes = ${nodeArrayExpression(children)};`,
    "  return nodes;",
    "}, () => {",
    ...fallbackNodes.code.map((line) => `  ${line}`),
    `  Element[] fallbackNodes = ${nodeArrayExpression(fallbackNodes)};`,
...
function lowerSwitch(node, context, parentVariable, namespace = "html") {
  ...
  for (const match of matches) {
    ...
    branchFunctions.push(
      [
        ...
        `  Element[] nodes = ${nodeArrayExpression(branch)};`,
        "  return nodes;",
        "}",
      ].join(" "),
    );
  ...
  return {
    ...
    code: [
      `(func()->bool)[] ${matchFunctions} = [${conditions.join(", ")}];`,
      `(func()->Element[])[] ${childrenFunctions} = [${branchFunctions.join(", ")}];`,
      `dynamicSwitch(region(${parentVariable}), ${matchFunctions}, ${childrenFunctions}, () => {`,
      ...fallbackNodes.code.map((line) => `  ${line}`),
      `  Element[] fallbackNodes = ${nodeArrayExpression(fallbackNodes)};`,
      "  return fallbackNodes;",
      "});",
...
function lowerErrorBoundary(node, context, parentVariable, namespace = "html") {
  ...
  return {
    varName: parentVariable,
    code: [
      `dynamicErrored(region(${parentVariable}), () => {`,
      ...children.code.map((line) => `  ${line}`),
      `  Element[] nodes = ${nodeArrayExpression(children)};`,
      "  return nodes;",
...
function lowerSuspense(node, context, parentVariable, namespace = "html") {
  ...
  return {
    varName: parentVariable,
    code: [
      `dynamicLoading(region(${parentVariable}), () => {`,
      ...children.code.map((line) => `  ${line}`),
      `  Element[] nodes = ${nodeArrayExpression(children)};`,
      "  return nodes;",
      "}, () => {",
...
function lowerDynamic(node, context, parentVariable, namespace = "html") {
  ...
  return {
    ...
    code: [
      ...code,
      `dynamicValue(${parentVariable}, () => ${component.trim()}, ${props}, (Element ${variable}) => {`,
...
function lowerPortal(node, context, parentVariable, namespace = "html") {
  ...
  return {
    varName: parentVariable,
    code: [
      `portalNodes(${parentVariable}, ${mount}, ${svg}, ${useShadow}, (Element node) => { ${ref ? `(${ref.trim()})(node);` : ""} }, () => {`,
      ...children.code.map((line) => `  ${line}`),
      `  Element[] nodes = ${nodeArrayExpression(children)};`,
      "  return nodes;",
      "});",
...
function lowerNodeArray(children, context, namespace, label) {
  ...
  for (const child of children) {
    if (child.type === "text") {
      ...
      code.push(`Element ${variable} = text(${JSON.stringify(value)});`);
      ...
      code.push(
        hasCallExpression(expression)
          ? `Element ${variable} = reactiveText(() => "" + (${expression}));`
          : `Element ${variable} = text("" + (${expression}));`,
      );
      groups.push({ variable, array: false });
    } else if (child.type === "element" && !isComponent(child.tag)) {
...
function lowerBuiltinNodeGroup(node, context, namespace) {
  ...
  return {
    ...
    code: [
      ...
      `Element[] ${nodes} = childNodes(${fragment});`,
    ],
  };
}
...
function lowerFor(node, context, parentVariable, namespace = "html") {
  ...
  const keyed = propertyExpression(node, "keyed");
  ...
  const customKey = keyed && keyed.trim() !== "true" && keyed.trim() !== "false";
  const singleValue =
    !parsed.hasIndex &&
    !fallback &&
    item.groups.length === 1 &&
    !item.groups[0].array;
  if (customKey && singleValue) {
    return {
      varName: parentVariable,
      code: [
        `dynamicKeyedValue(${parentVariable}, ${each.trim()}, ${keyed.trim()}, (${parsed.typeName} ${parsed.item}) => {`,
  ...
  if (customKey) {
    ...
    return {
      varName: parentVariable,
      code: [
        `dynamicKeyedNodes(${parentVariable}, ${each.trim()}, ${keyed.trim()}, (${parameters}) => {`,
        ...item.code.map((line) => `  ${line}`),
        `  Element[] nodes = ${nodeArrayExpression(item)};`,
        "  return nodes;",
        "});",
      ],
    };
...
function lowerRepeat(node, context, parentVariable, namespace = "html") {
  if (!parentVariable) throw new Error("Repeat requires a parent element");
  const count = propertyExpression(node, "count");
  if (!count) throw new Error("Repeat requires count={...}");
  const childFunction = node.children.find((child) => child.type === "expr");
  if (!childFunction) throw new Error("Repeat requires {(int index) => <...>} child");
  const arrow = parseTypedArrow(childFunction.value, "Repeat");
  if (!arrow || arrow.parameters.length !== 1) {
    throw new Error("Repeat requires {(int index) => <...>} child");
  }
  const [parameter] = arrow.parameters;
  if (compactType(parameter.type) !== "int") {
    throw new Error("Repeat index parameter must have type int");
  }
  const element = parseJsx(unwrapArrowElement(arrow.body, "Repeat"), 0).node;
  const item = lowerNodeArray([element], context, namespace, "Repeat row");
  const singleValue = item.groups.length === 1 && !item.groups[0].array;
  if (singleValue) {
    return {
      varName: parentVariable,
      code: [
        `dynamicRepeat(${parentVariable}, () => ${count.trim()}, (int ${parameter.name}) => {`,
        ...item.code.map((line) => `  ${line}`),
        `  return ${item.groups[0].variable};`,
        "});",
      ],
  ...
  return {
    varName: parentVariable,
    code: [
      `dynamicRepeatNodes(${parentVariable}, () => ${count.trim()}, (int ${parameter.name}) => {`,
      ...item.code.map((line) => `  ${line}`),
      `  Element[] nodes = ${nodeArrayExpression(item)};`,
      "  return nodes;",
      "});",
...
function lowerDynamicUpdate(variable, property) {
  if (
    (property.name === "classList" || property.name === "class") &&
    property.type === "expr" &&
